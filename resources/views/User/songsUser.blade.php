...
<body id="grid1">
  <script>
      ...
      let params=(new URL(document.location)).searchParams;
       ...
       if(errors=="1"){
         alert("Error - Las canciones tienen el mismo nombre a editar.")
       }
       if(errors=="2"){
         alert("Error - Id inexistente.")
       }
       if(errors=="3"){
         alert("Error - Datos erroneos o faltantes.")
       }
       if(pass=="1"){
    ...
    <table class="table table-dark table-striped mt-4">
        <p style="font-family: 'Roboto', sans-serif; font-size: x-large;">Canciones</p>
        ...
        <tbody>
            @foreach($cancionesUser as $cancionesUser)
            <tr>
                <td>{{$cancionesUser->id}}</td>
                <td>{{$cancionesUser->nombre_cancion}}</td>
                <td>{{$cancionesUser->likes}}</td>
                <td>{{$cancionesUser->reproducciones}}</td>
                <td>
                    <a href="/song/player?namesong={{$cancionesUser->nombre_cancion}}&urlsong={{$cancionesUser->url_cancion}}" class="btn btn-info">Escuchar</a>
                    @csrf
                    @method('POST')
